...
2026-05-13 — another long autonomous polish run, ~50 features merged on `develop`. First half: BMR/TDEE onboarding goals, water tracking on Today, macro pie chart, recipe duplicate, theme picker, Help/FAQ sheet, Spotlight indexing recipes, app-version footer, recipe rating UI + ratingDesc sort, MacroSplit presets in EditGoals, onboarding restart from Profile, Today date scrubbing with chevrons + DateJumpSheet, meal photo zoom (pinch + pan), achievement detail sheet + share card, calibration reset, friend share-link in MyCodeSheet, recipe ingredient cook-mode checkboxes, cultural event dismissal, streak explanation modal, achievements earned-only filter, food seed 104 → 129, custom food creation in Quick DB, avatar reset, Today date picker jump, Quick DB long-press quick-log, recipe cooking timer, weight goal RuleMark + editor, CSV export date range, "Pomiń na razie" onboarding skip, weight row tap-to-edit, MealEntry tag autocomplete, lifetime stats total kcal, coach history helpful filter, reset Quick DB recents. Second half: editable daily water goal, BMI card on weight view, tap-Cel calorie-goal editor on Today, "N dni z nami" precise day count, MealRepository.updateConsumedAt + DatePicker in detail sheet, undo window 5s → 8s, calorie ring goes gold at goal / red past 120%, MealRepository.cleanupOrphanedPhotos + Profile sweep button, recipe ingredient clipboard copy, WeightService.sevenDayAverageKg in summary, tap-tab-again scroll-to-top on Today, recipe URL import clipboard auto-paste, total cooked-recipes lifetime stat, MealEntry.rating + 1-5 star UI in detail sheet, rating chip on timeline row + avg rating in lifetime stats, CSV export rating + tags columns, longest-streak in lifetime footer, cultural events catalog 6 → 9 (Dzień Matki, Dzień Dziecka, truskawkowy sezon), kg/tyg. rate-of-change in weight summary, camera badge on timeline rows for meals with photos, lifetime water total in stats, notification-status hint on Preferences row, photo cache size next to orphan-sweep row, "Tylko moje" filter chip in Quick DB, recipe-list filtered-count + clear-filters link. **Mega-batch wrap**: 3 more cultural events (Dzień Babci, Niepodległości, Świętojańska) with table-driven occurrence resolver, 2 more Coach rules (morning protein, afternoon momentum), food seed 129 → 160, voice meal multi-item splitting on i/oraz/plus/comma, activity heatmap tap-a-day drill-down sheet, RecipeListView "Co podobnego?" carousel (shared-token engine + tests), RecipeModificationEngine (M3.3 prep, 4 intents × per-ingredient swap rules + sheet UI), Onboarding dietary preferences step (User row gains comma-joined `dietaryPreferencesRaw` + 6-case enum), FoodLabelScanner + NutritionLabelParser (M2.5 prep, local Vision OCR), DataBundleExportService (JSON + CSV + photos → ZIP via NSFileCoordinator), VoiceOver labels on calorie ring + macro rows + stat tiles + meal timeline rows. **Final closeout batch**: voice meal preview ("Co zapiszemy" parsed-items card live updates as transcript edits), FoodLabelScanner wired into Quick DB via LabelScannerSheet → CustomFoodFormSheet prefill (M2.5 user-facing surface), CoachMemoryNote @Model + CoachMemoryStore (M3.2 prep — observation/preference/milestone/goal with case-insensitive upsert + confidence averaging), per-suggestion clipboard copy + bullet-list ShareLink on recipe modifications. **Fine-polish wrap-up**: 5 decision-log entries appended to ARCHITECTURE.md (append-only migrations, M2.5 local Vision, M3.2 memory store, M3.3 modifications, NSFileCoordinator ZIP), 7 more voice-parser edge-case tests (comma decimal, gram suffixes, "kalorie", word-internal "i", "plus", empty fragments), haptic gaps closed (voice commit / similar-recipe tap / day-meal row tap), A11yID catalog expanded for new surfaces. **97 feature commits today**. swiftlint --strict clean, swift-format clean, unit tests green (1 pre-existing UI flake unchanged).
